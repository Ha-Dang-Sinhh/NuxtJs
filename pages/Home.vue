<template>
  <div>
    ...
    <form action="" @submit.prevent="searchPost()">
      <div class=" bg-white rounded-md px-6 py-10 max-w-2xl mt-10 mx-auto">
        <h1 class="text-center text-2xl font-bold text-gray-500 mb-10">Search POST</h1>
        <div class="space-y-4">
          <div>
            <label for="title" class="text-lx font-serif">Title:</label>
            <input type="text" placeholder="title" v-model="titleSearch" id="title" class="ml-2 outline-none py-1 px-2 text-md border-2 rounded-md" />
          </div>
          <button class=" px-6 py-2 mx-auto block rounded-md text-lg font-semibold text-indigo-100 bg-indigo-600 " >Search POST</button>
        </div>
      </div>
    </form>
      ...
      <ul class="flex flex-col p-4">
        <li class="border-gray-400 flex flex-row">
          ...
          >
            ...
            <nuxt-link :to="{ name: 'Edit', params: { id: item.id }}">
...
export default {
  ...
  methods: {
    ...
    async searchPost(){
      let unSub = null;
      const collectionRef = collection(db, 'todos');
      const collectionQuery = query(collectionRef, where('title', '==',this.titleSearch ));
      try {
